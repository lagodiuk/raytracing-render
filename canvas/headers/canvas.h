...
static inline void clear_canvas(Canvas * canv) {
    memset(canv->data, 0, canv->w * canv->h * sizeof(Color));
}

static inline void release(Canvas * c) {
	free(c->data);
	free(c);
}

static inline void set_pixel(int x, int y, Color c, Canvas * canv) {
	int offs = y * canv->w + x;
	canv->data[offs] = c;
}

static inline Color get_pixel(int x, int y, Canvas * canv) {
	int offs = y * canv->w + x;
